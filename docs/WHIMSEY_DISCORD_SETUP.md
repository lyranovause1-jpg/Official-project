...
| --- | --- | --- | --- |
| 0:00 – 0:45 | **Phase A — Critical reading** _(just sections 2, 4, skim 28 intro — skip the rest for now, come back to reference sections later)_ | 📖 READ | 45 min |
...
## PRE-MINT TIMING (read this first!) 🌷💗
...
- [ ]  9.4 Create category `🎫 | TICKETS` (private). Add channels per section 25 (`#ticket-archive`, `#ticket-logs`, etc.). Note: `#open-tickets` lives in `☁️ | SUPPORT` — it is the public entry point where members click the Ticket Tool button. The private ticket channels spawn inside `🎫 | TICKETS` automatically.
...
- [ ]  13.3 In Vulcan's dashboard (vulcan-bot), connect your $CNDY contract address.
...
- [ ]  C-2.1 **Daily recap** at 23:55 IST → `#momentum-daily-recap`.
- [ ]  C-2.2 **Daily on-chain rollup** (sales/listings/whales/mints/volume) → `#momentum-collection-feed` (NFT Tracker bot, configured in step C-3).
- [ ]  C-2.3 **Weekly recap** every Sunday 23:55 IST → `#momentum-weekly-recap`.
- [ ]  C-2.4 **Monthly recap** last day of month 23:55 IST → `#momentum-monthly-recap`.
- [ ]  C-2.5 **Weekly team-pulse leaderboard** every Monday 12:00 IST → `#momentum-team-pulse`.
- [ ]  C-2.6 **Daily holder snapshot** 00:05 IST → `#momentum-holder-snapshot`.
- [ ]  C-2.7 **Live server stats pinboard** (auto-updated member/verified/holder counts) → `#momentum-server-stats`.
- [ ]  C-2.8 **Twitter/X feed mirror** (webhook auto-posts your official tweets) → `#momentum-twitter-feed`.
...
01. The 18-step setup order (do these in this exact order)
02. The member journey — the experience your permissions must produce
03. Server-wide safety configuration (Safety Setup, AutoMod, Onboarding, 2FA)
04. Role hierarchy — the exact vertical order (top → bottom)
05. Role 1 — `Admin 💗` — every permission, ON/OFF
06. Role 2 — `Moderator ☁️` — every permission, ON/OFF
07. Role 3 — `Holder 🌌` — every permission, ON/OFF
08. Role 4 — `Verified 🩵` — every permission, ON/OFF
09. `@everyone` — every permission, ON/OFF (the server-wide lockdown)
...
- _Why:_ blocks fresh throwaway accounts (the #1 vector for raid bots and scam bots in NFT servers).

### 3.2) Explicit Media Filter

Set to **Filter messages from all members**.

- _Why:_ auto-deletes any image Discord's classifier flags as explicit. Your community will include minors and a clean filter is mandatory for Server Discovery later.

### 3.3) DM Spam Filter

Set to **Filter all direct messages**.

- _Why:_ auto-scans DMs for spam/scam content. Doesn't stop DMs entirely (you can't, only the recipient can), but flags obvious scams.
...
- _Why:_ Moderators MUST have 2FA enabled on their personal Discord account or they can't kick/ban/delete. Stops compromised mod accounts from nuking the server.
...
5.   Vulcan                ← bot role auto-created when you invite Vulcan
...
| Permission | ON / OFF |
| --- | --- |
| Create Invite | **OFF** _(prevent unsanctioned invite spam from compromised holder accounts)_ |
| Change Nickname | **ON** |
...
| Create Private Threads | **OFF** _(prevents private group spaces that mods can't see)_ |
| Embed Links | **ON** |
| Attach Files | **ON** |
| Add Reactions | **ON** |
...
| Permission | ON / OFF | Reasoning |
| --- | --- | --- |
| View Channels | **OFF** | Hides every channel by default. We will explicitly Allow ✅ View Channels for `@everyone` ONLY in the \`💗 |
| Manage Channels | **OFF** |  |
| Manage Roles | **OFF** |  |
| Create Expressions | **OFF** |  |
| Manage Expressions | **OFF** |  |
...
## 10) CREATE THE 4 PRIVATE CATEGORIES + ALL CHANNELS ✨ UPDATED


> **✨ NEW (Report Sheet):** Two additional channels have been added to the 🔒 | STAFF category for WHIMSEY AI:
>
> - `#whimsey-ai-communicate` — The exclusive private channel between you (the server owner) and WHIMSEY AI. Only Admin 💗 and WHIMSEY AI can see or post here. This is your CEO briefing room.
> - `#message-confirmation` — Where WHIMSEY AI queues messages destined for public channels, waiting for your ✅ approval before they post. In Autopilot Mode, this gate is bypassed.
>
> See Section 35 for the complete WHIMSEY AI setup guide.
...
## 11) BOT 1 — Auth (Captcha Verification)
...
## 12) BOT 2 — VULCAN (NFT Holder Verification) 🔄 UPDATED


> **🔄 UPDATED (Report Sheet):** **Vulcan has been replaced with Vulcan** for holder verification. Vulcan is the bot that handles NFT wallet verification and grants the `Holder 🌌` role. All setup instructions below are for Vulcan. Vulcan occupies the same role hierarchy position (#5), requires the same permissions (Manage Roles, View Channels, Send Messages, Embed Links, Manage Messages, Read Message History, Use Application Commands), and binds to the same `#holder-verify` channel.
...
## 14) BOT 4 — Carl-bot (Autopilot Brain)
...
## 16) CATEGORY 1 — 💗 | VERIFY 🔄 UPDATED


> **🔄 UPDATED (Report Sheet):** The VERIFY category visibility has been changed. Unlike the original guide which hides this category after verification, the 💗 | VERIFY category is now **permanently visible to ALL members** — including those who have already verified. Members can always return to `#access-info` and `#verify`. To implement this: **remove the `Verified 🩵` Deny ❌ on View Channels** from the section 16 permission table. Leave it as Neutral ➖ for Verified 🩵 (so they inherit the @everyone Allow ✅ and can always see the category).



**Visibility goal (UPDATED):** Now visible to **ALL members at all times**, including those who have already verified. Remove the `Verified 🩵` Deny ❌ on View Channels — leave it as Neutral ➖ so verified members can still see and return to this category.
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Auth bot |
| --- | --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny _(overridden Allow ✅ in `#verify` only — see section 26)_ | ➖ | ➖ | ➖ | ➖ | ➖ |
| Use Activities | ❌ Deny | ➖ | ➖ | ➖ | ➖ | ➖ |
...
| Read Message History | ✅ Allow _(safe; needed for everyone who can see it)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| --- | --- | --- | --- | --- | --- | --- |
| View Channels | ❌ Deny | ❌ Deny _(overridden Allow ✅ on `#holder-verify` only)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Vulcan |
| --- | --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny | ❌ Deny _(overridden Allow ✅ on `#holder-verify`)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Activities | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ |
| Use External Apps | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ |
...
| --- | --- | --- | --- | --- | --- |
| Send Messages and Create Posts | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Send Messages in Threads and Posts | ❌ Deny | ✅ Allow _(so they can discuss inside event threads)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Create Public Threads | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Create Private Threads | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Embed Links | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Attach Files | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 |
| --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny | ✅ Allow _(needed for entering giveaways/polls)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Activities | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Use External Apps | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ |
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Ticket Tool |
| --- | --- | --- | --- | --- | --- | --- |
| Send Messages and Create Posts | ❌ Deny | ❌ Deny _(overridden Allow ✅ on `#support` — see section 26)_ | ❌ Deny _(overridden Allow ✅ on `#support`)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Send Messages in Threads and Posts | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow | ➖ |
...
| Send Text-to-speech Messages | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ |
| Send Voice Messages | ❌ Deny | ✅ Allow _(useful in `#support` if they want to explain a complex bug)_ | ✅ Allow | ✅ Allow | ✅ Allow | ➖ |
| Create Polls | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ |
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Ticket Tool |
| --- | --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny | ✅ Allow _(needed to click the Ticket Tool button)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Activities | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ |
| Use External Apps | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ |
...
| Manage Webhooks | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ✅ Allow _(Carl-bot uses webhooks for logs)_ |
...
| Mention @everyone, @here and All Roles | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(team-wide pings)_ | ✅ Allow | ➖ |
| Manage Messages | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Pin Messages | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ |
...
## 24A) CATEGORY 11 — 📋 | AUDITS ✨ NEW BOT LOG CHANNELS


> **🔄 UPDATED (Report Sheet):** In addition to the 20 event-based audit channels above, 4 **dedicated bot log channels** have been added to this category — one per bot. These give you a clean per-bot view alongside the event-based view:
>
> | Channel | Bot | What it logs |
> | --- | --- | --- |
> | `#log-auth-bot` | Auth | Every verification attempt (success, fail, captcha result) |
> | `#log-vulcan` | Vulcan | Every holder wallet verification event (replaces old Collab.Land log) |
> | `#log-ticket-tool` | Ticket Tool | Every ticket opened, closed, escalated, transcript posted |
> | `#log-carlbot` | Carl-bot | Every Carl-bot autopilot action, scheduled post, and decision |
>
> Add these 4 channels to the 📋 | AUDITS category and apply the same category-level permissions (staff-only, bots can write).


## 24A) CATEGORY 10 — 📋 | AUDITS ✨ NEW BOT LOG CHANNELS
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Carl-bot | Vulcan | Ticket Tool |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Send Messages and Create Posts | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny _(humans must NOT pollute the audit trail)_ | ✅ Allow _(only Admin can post a manual annotation if needed)_ | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Add Reactions | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(let mods react ✅ "investigated" / 👀 "watching" on log entries)_ | ✅ Allow | ➖ | ➖ | ➖ |
| Use External Emojis | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ | ➖ | ➖ |
| Use External Stickers | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ | ➖ | ➖ |
| Mention @everyone, @here and All Roles | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow _(so Carl-bot can ping @Moderator on a critical log entry — e.g. mass-ban detected)_ | ➖ | ➖ |
| Manage Messages | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny _(NEVER let mods delete log entries)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Carl-bot | Vulcan | Ticket Tool |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny _(no slash commands here — keeps the trail pure)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Manage Channels | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ | ➖ |
| Manage Permissions | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ | ➖ |
| Manage Webhooks | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ✅ Allow _(for Twitter-feed webhook + scheduled embeds)_ | ➖ |

### Text Channel Permissions

| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Carl-bot | Vulcan |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Send Messages and Create Posts | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(mods can comment on a recap, e.g. "spike from the AMA")_ | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Use External Emojis | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ➖ | ➖ |
| Mention @everyone, @here and All Roles | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(useful for "@Moderator — recap is up")_ | ✅ Allow | ✅ Allow | ➖ |
| Manage Messages | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Pin Messages | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(pin the most important monthly recap)_ | ✅ Allow | ➖ | ➖ |
| Read Message History | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Create Polls | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(quick team votes on what to feature)_ | ✅ Allow | ➖ | ➖ |

### Apps Permissions

| Permission | @everyone | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 | Carl-bot | Vulcan |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Use Application Commands | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow _(e.g. `/list-holders` posts here)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Activities | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ |
| Use External Apps | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ➖ |
...
## 25) CATEGORY 12 — 🎫 | TICKETS 🔄 UPDATED


> **🔄 UPDATED (Report Sheet):** The 🎫 | TICKETS category has been clarified. It now explicitly contains:
>
> - `#open-tickets` — Entry point (visible to Verified + Holder). The Ticket Tool panel lives here. Clicking creates a private ticket channel.
> - `#ticket-logs` — Live feed of all ticket events (opened, closed, escalated). Staff-only.
> - `#ticket-archive` — Where ALL closed-ticket transcripts are permanently stored by Ticket Tool. Both currently-open and historically-closed tickets have their transcripts here. Staff-only.
>
> This gives the team a complete view of all support activity, past and present.
...
| --- | --- | --- | --- | --- | --- | --- |
| View Channels | ❌ Deny | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Manage Channels | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ✅ Allow _(needs to spawn ticket channels)_ |
| Manage Permissions | ❌ Deny | ❌ Deny | ❌ Deny | ❌ Deny | ➖ | ✅ Allow _(needs to add the ticket-opener's overwrite)_ |
...
| Use Application Commands | ✅ Allow _(critical — so the Auth button works)_ | ✅ Allow | ✅ Allow |
...
| --- | --- | --- | --- | --- |
| Send Messages | ❌ Deny | ❌ Deny | ❌ Deny _(even mods don't post here — only Admin)_ | ✅ Allow |
| Add Reactions | ✅ Allow _(let them ✅ react to acknowledge)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Manage Messages | ❌ Deny | ❌ Deny | ✅ Allow _(mods can edit/delete obsolete pinned content)_ | ✅ Allow |
...
| --- | --- | --- | --- | --- |
| Send Messages | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow |
| Send Messages in Threads | ✅ Allow _(let them discuss the daily Whimsey inside threads)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Create Public Threads | ❌ Deny _(only staff opens the daily thread)_ | ❌ Deny | ✅ Allow | ✅ Allow |
...
| --- | --- | --- | --- | --- | --- |
| View Channel | ✅ Allow _(critical — overrides the category-level Deny on Verified)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Read Message History | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Send Messages | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Embed Links | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Application Commands | ✅ Allow _(critical — so the Vulcan button works)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Mention @everyone, @here and All Roles | ❌ Deny | ❌ Deny | ✅ Allow _(so mods can ping `@Holder`)_ | ✅ Allow |
...
| Send Messages | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Embed Links | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ✅ Allow _(reacting is how you enter)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Permission | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 |
| --- | --- | --- | --- | --- |
| Send Messages | ✅ Allow _(overrides category Deny)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Embed Links | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Attach Files | ✅ Allow _(so they can upload screenshots of issues)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Application Commands | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Create Public Threads | ✅ Allow _(so each support question can have its own thread)_ | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Permission | Verified 🩵 | Holder 🌌 | Moderator ☁️ | Admin 💗 |
| --- | --- | --- | --- | --- |
| Send Messages | ❌ Deny | ❌ Deny | ❌ Deny _(prevents mods from accidentally turning the FAQ into a chat)_ | ✅ Allow |
| Add Reactions | ✅ Allow _(let people upvote which FAQs were helpful)_ | ✅ Allow | ✅ Allow | ✅ Allow |
| Manage Messages | ❌ Deny | ❌ Deny | ✅ Allow _(mods can edit/pin)_ | ✅ Allow |
...
| Send Messages | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Mention @everyone, @here and All Roles | ❌ Deny | ❌ Deny | ✅ Allow _(scam alerts SHOULD ping everyone — they're urgent)_ | ✅ Allow | ✅ Allow |
...
| Embed Links | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Add Reactions | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
| Use Application Commands | ✅ Allow _(critical — the Ticket Tool button)_ | ✅ Allow | ✅ Allow | ✅ Allow | ✅ Allow |
| Manage Messages | ❌ Deny | ❌ Deny | ✅ Allow | ✅ Allow | ✅ Allow |
...
| Permission | Moderator ☁️ | Admin 💗 |
| --- | --- | --- |
| Manage Webhooks | ❌ Deny | ✅ Allow _(only Admin manages webhooks for security)_ |
...
| Member avatar changed | `#audit-member-updates` _(only flag for staff + Holders to detect account hijacks)_ |
...
| Event | Destination |
| --- | --- |
| `Holder 🌌` role added (Carl-bot detects role-add events) | `#audit-holder-changes` _(mirrors Vulcan's grant for easy auditing in one place)_ |
| `Holder 🌌` role removed | `#audit-holder-changes` |
| `Verified 🩵` role added | `#audit-holder-changes` _(optional, gives a single feed of "membership state changes")_ |
| Server boost added | `#audit-boosts` _(post a celebration message — boosters love being recognized)_ |
...
01. **Server icon, splash, banner.** Splash + banner need Boost Level 1 / 2.
02. **Server invite splash image** (the preview image people see on the invite link before they accept).
03. **Vanity URL**`discord.gg/whimsey` (Boost Level 3 — needs 14 boosts).
04. **Custom emoji + stickers** — upload 30–50 WHIMSEY-themed reactions. This becomes your community's "language".
05. **Custom soundboard sounds** — 8 short signature sounds for hype moments (Boost Level 2 / 3).
06. **Onboarding** with interest tags — see section 3.5.
07. **Native Discord Events** — pre-schedule mints, AMAs, giveaways using **Server → Events** so members can RSVP and get reminders.
08. **Apply for Server Discovery** once you have ≥1000 members and Onboarding set up — major organic growth lever.
09. **Apply for the Verified Server checkmark** once eligible — huge trust signal.
...
1. Check Vulcan's status page (status.vulcan-bot). If their service is down, this is global, not your problem.
...
1. Check the bot's status page (carl.gg/status, status.vulcan-bot, etc).
...
### 34.5) vulcan-bot — HOLDER VERIFICATION AUTOPILOT

#### 34.5.1) Setup

1. Invite Vulcan from vulcan-bot/discord. They'll walk you through their guided setup.
2. Move Vulcan role to position **#5** (under Auth, above Ticket Tool).
...
**`set_channel_permissions` supports these permission names:**`VIEW_CHANNEL`, `SEND_MESSAGES`, `READ_MESSAGE_HISTORY`, `ADD_REACTIONS`, `EMBED_LINKS`, `ATTACH_FILES`, `MANAGE_MESSAGES`, `USE_APPLICATION_COMMANDS`, `CONNECT`, `SPEAK`, `SEND_MESSAGES_IN_THREADS`
...
| `update_doc_section` | Add or replace sections in WHIMSEY\_DISCORD\_SETUP.md | `action`, `content`, `sectionHeading`, `summary` |
...
### 37.1) THE UPDATE\_DOC\_SECTION TOOL
...
> → WHIMSEY AI uses replace\_section to rewrite that section.
